...
![[Pasted image 20251114102214.png]]

Ahora que tenemos la respuesta, podemos hacer clic en _Continuar_ para reanudar la ejecución. Podemos enviar otra solicitud con la respuesta correcta. Cuando hayamos terminado con la conexión del depurador, podemos hacer clic en _Desconectar_ . Luego podemos ir a nuestra terminal y presionar C + C para detener la aplicación Java.

Aunque la funcionalidad de esta aplicación es sencilla, este ejercicio demuestra la utilidad de la depuración remota al desarrollar exploits para aplicaciones web. Esta sección sirvió como introducción a la depuración remota, y utilizaremos este proceso ampliamente a lo largo del curso.

#### Ceremonias

1. Repita los pasos descritos en esta sección y familiarícese con la depuración remota en VS Code.
2. Inicie la máquina de depuración del laboratorio y ejecute NumberGame.jar desde ella. Depure la aplicación de forma remota desde su máquina Kali local. Encontrará la dirección IP y las credenciales de la máquina de depuración en su panel de control. Recuerde habilitar las conexiones remotas para la depuración al ejecutar el archivo JAR.

## 1.6. Conclusión

En este módulo, abordamos algunas de las herramientas y técnicas fundamentales utilizadas para la evaluación de aplicaciones web de caja blanca. Revisamos cómo usar Burp Suite para inspeccionar y modificar el tráfico HTTP. Además, sentamos las bases para el análisis de código fuente demostrando cómo recuperar código fuente Java y .NET, y comenzamos a crear nuestra propia metodología para analizar dicho código en busca de vulnerabilidades. Ahora que hemos demostrado estas técnicas básicas, las aprovecharemos para examinar diversas aplicaciones vulnerables y explorar las distintas vulnerabilidades que contienen.